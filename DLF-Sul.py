...
def encode(p_string, n_string):
    ...
    return x_BE_p, x_62_p, x_AAI_p, x_BE_n, x_62_n, x_AAI_n
...
def train_data():
    ...
    (
        ...
        input_batch_62_n,
        input_batch_AAI_n,
    ) = encode(train_p_string, train_n_string)
    ...
    # print(target_batch)
    return (
        torch.FloatTensor(input_batch_BE_p),
        torch.FloatTensor(input_batch_62_p),
        torch.FloatTensor(input_batch_AAI_p),
        torch.FloatTensor(input_batch_BE_n),
        torch.FloatTensor(input_batch_62_n),
        torch.FloatTensor(input_batch_AAI_n),
...
def validate_data():
    ...
    (
        ...
        input_batch_62_n,
        input_batch_AAI_n,
    ) = encode(validate_p_string, validate_n_string)
...
def test_data():
    ...
    (
        ...
        input_batch_62_n,
        input_batch_AAI_n,
    ) = encode(test_p_string, test_n_string)
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.blstm = nn.LSTM(
            input_size=54, hidden_size=n_hidden, bidirectional=True, batch_first=True
        )
        ...
        self.FC = nn.Sequential(
            nn.Linear(3224, 64), nn.Dropout(0.5), nn.Linear(64, 2), torch.nn.Sigmoid()
        )
...
def train():
    ...
    for epoch in tqdm(range(epochs)):
        for x_BE_p, x_62_p, x_AAI_p, x_BE_n, x_62_n, x_AAI_n, y_p, y_n in trainloader:
            model.train()
            # print(x_p.shape)
            # print(x)
            # x_p= x_p.cuda()
